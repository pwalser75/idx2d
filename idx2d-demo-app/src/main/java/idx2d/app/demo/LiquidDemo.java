...
public final class LiquidDemo extends DemoView {
    ...
    private static final long PHYSICS_STEP_MILLIS = 10;

    private Texture output;
    private long time = 0;
    private long lastTime = 0;
    private long physicsBudget = 0;
    private long lastRandomDropWindow = -1;
    ...
    private void applyEffect() {
        time = elapsedMillis();
        physicsBudget += time - lastTime;
        lastTime = time;

        while (physicsBudget >= PHYSICS_STEP_MILLIS) {
            physicsBudget -= PHYSICS_STEP_MILLIS;
            if (autodrop) {
                if (random) {
                    long window = time / 200;
                    if (window != lastRandomDropWindow) {
                        lastRandomDropWindow = window;
