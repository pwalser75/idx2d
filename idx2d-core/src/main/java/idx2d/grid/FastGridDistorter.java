...
public class FastGridDistorter implements GridDistorter
{
	public int width=0;
	public int height=0;
	...
	private int offset;
	private int xpos,ypos,tx,ty,tw,th;
	private int dtx,dty;
	...
	// C O N S T R U C T O R S
		...
		public FastGridDistorter(int w,int h)
		{
			width=w;
			height=h;
	...
	// P R I V A T E   M E T H O D S
		...
		private void renderLine(int x, int y, int txL, int txR, int tyL, int tyR)
		{
			offset=y*width+x;
			tx=(txL%(tw<<16))+(tw<<20);
			ty=(tyL%(th<<16))+(th<<20);
			...
			for(int i=0;i<8;i++)
			{
				pixel[offset+i]=source[((tx>>16)%tw)+((ty>>16)%th)*tw];
				tx+=dtx;
				ty+=dty;
			}
